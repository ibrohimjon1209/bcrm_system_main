...
const Home = () => {
  ...
  const statCards = [
    { label: 'Tushum',    icon: CurrencyDollar, path: '/reports', value: `$${fmt(revUSD)}`,      sub: `${fmt(revUZS)} so'm`,                          className: 'col-span-2 md:col-span-1' },
    { label: 'Foyda',     icon: TrendUp,         path: '/reports', value: `$${fmt(profUSD)}`,     sub: `${fmt(profUZS)} so'm`,                          className: 'col-span-2 md:col-span-1' },
    { label: 'Xarajat',   icon: CreditCard,      path: '/reports', value: `$${fmt(costUSD)}`,     sub: `${fmt(costUZS)} so'm`,                          className: 'col-span-2 md:col-span-1' },
    { label: 'Xarid',     icon: Truck,           path: '/reports', value: `$${fmt(purchaseUSD)}`, sub: `${fmt(purchaseUZS)} so'm | ${purchasesCount} ta`, className: 'col-span-2 md:col-span-1' },
    { label: 'Qarz',      icon: CreditCard,      onClick: () => navigate('/customers', { state: { filter: 'Qarzdorlar' } }), value: `$${fmt(debtUSD)}`, sub: `${fmt(debtUZS)} so'm | ${debtorsCount} ta`, className: 'col-span-2 md:col-span-1' },
    { label: 'Sotuvlar',  icon: ShoppingCart,    path: '/reports', value: salesCount,             sub: `${purchasesCount} ta xarid`,                    className: 'col-span-1' },
    { label: 'Mahsulotlar',icon: Package,        path: '/warehouse',value: totalProductsCount,    sub: 'dona',                                          className: 'col-span-1' },
  ];

  const quickActions = [
    { label: 'Sotuv',    icon: ShoppingCart, path: '/sales',     bg: 'bg-blue-50',    color: 'text-[#1447E6]'  },
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-28 md:pb-8">
      {/* Header */}
      <div className="bg-gradient-to-br from-[#1447E6] to-[#0F3CC7] px-5 md:px-8 pt-10 pb-16 md:pb-10 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex justify-between items-center mb-6 relative z-10">
            <div>
              <h1 className="text-white text-xl md:text-2xl font-black leading-tight">Xush kelibsiz!</h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 relative z-10">
            {statCards.map(({ label, path, onClick, value, sub, className, icon: Icon }) => {
              const content = (
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-3 md:p-4 border border-white/20 h-full hover:bg-white/15 transition-colors active:scale-[0.98] transition-all">
                  ...
                  <p className="text-blue-200 text-xs font-bold mt-0.5">{sub}</p>
                </div>
              );
              return path ? (
                <Link key={label} to={path} className={className}>{content}</Link>
              ) : (
                <button key={label} onClick={onClick} className={`${className} text-left`}>{content}</button>
              );
            })}
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      <div className={`w-12 h-12 md:w-14 md:h-14 ${action.bg} rounded-2xl flex items-center justify-center`}>
                        <Icon className={`w-5 h-5 md:w-6 md:h-6 ${action.color}`} weight="bold" />
                      </div>
            ...
            {chartData.length > 1 && (
              <div className="p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <TrendUp className="text-[#1447E6] w-4 h-4" />
                </div>
