...
import { useCompany, useCompanyUsers } from '../../hooks/useCompany';

const SACompanyUsers = () => {
  const { id } = useParams();
  ...
  const { data, isLoading: usersLoading } = useCompanyUsers(id, { search, page, page_size: 20 });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-white text-xl font-black">Foydalanuvchilar</h1>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
        {usersLoading ? (
          ...
          <div className="text-center py-12">
            <FiUser className="text-gray-600 w-10 h-10 mx-auto mb-3" />
            <p className="text-gray-400 text-sm font-semibold">Foydalanuvchilar topilmadi</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-700">
                    {['Foydalanuvchi', 'Telefon', 'Rol', 'Holat'].map(h => (
                  ...
                  {users.map((u) => (
                    <tr key={u.id} className="hover:bg-gray-750 transition-colors">
                      <td className="px-4 py-3">
                        <div>
                          <p className="text-white text-sm font-bold">{u.full_name || u.username}</p>
                      ...
                      <td className="px-4 py-3 text-gray-300 text-sm">
                        <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${
                          u.role === 'owner' ? 'bg-purple-900/40 text-purple-400' :
                          u.role === 'admin' ? 'bg-blue-900/40 text-blue-400' :
                          'bg-gray-700 text-gray-400'
                        }`}>
                          {u.role === 'owner' ? 'Egasi' : u.role === 'admin' ? 'Admin' : u.role || 'Xodim'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${
