...
import { useCompany, useCompanyCustomers } from '../../hooks/useCompany';

const SACompanyCustomers = () => {
  const { id } = useParams();
  ...
  const { data, isLoading: customersLoading } = useCompanyCustomers(id, { search, page, page_size: 20 });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-white text-xl font-black">Mijozlar</h1>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
        {customersLoading ? (
          ...
          <div className="text-center py-12">
            <FiUsers className="text-gray-600 w-10 h-10 mx-auto mb-3" />
            <p className="text-gray-400 text-sm font-semibold">Mijozlar topilmadi</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-700">
                    {['Mijoz', 'Telefon', 'Manzil', 'Qarz', 'Holat'].map(h => (
                ...
                <tbody className="divide-y divide-gray-700">
                  {customers.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-750 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-300 text-sm">{c.address || '—'}</td>
                      <td className="px-4 py-3 text-gray-300 text-sm">
                        {(parseFloat(c.debt_uzs || 0) + parseFloat(c.debt_usd || 0)).toFixed(2)}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${
                          c.status === 'active' ? 'bg-green-900/40 text-green-400' : 'bg-gray-700 text-gray-400'
                        }`}>
...
export default SACompanyCustomers;