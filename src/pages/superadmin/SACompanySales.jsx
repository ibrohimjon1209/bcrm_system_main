...
import { FiArrowLeft, FiSearch, FiDollarSign } from 'react-icons/fi';
import { useCompany, useCompanySales } from '../../hooks/useCompany';
import { fmtMoney, fmtDate } from '../../utils/formatters';

const SACompanySales = () => {
  const { id } = useParams();
  ...
  const { data, isLoading: salesLoading } = useCompanySales(id, { search, page, page_size: 20 });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-white text-xl font-black">Sotuvlar</h1>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
          placeholder="Mijoz, to'lov usuli..."
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
        {salesLoading ? (
          ...
          <div className="text-center py-12">
            <FiDollarSign className="text-gray-600 w-10 h-10 mx-auto mb-3" />
            <p className="text-gray-400 text-sm font-semibold">Sotuvlar topilmadi</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-700">
                    {['ID', 'Mijoz', 'To\'lov', 'Sana', 'Holat'].map(h => (
                ...
                <tbody className="divide-y divide-gray-700">
                  {sales.map((s) => (
                    <tr key={s.id} className="hover:bg-gray-750 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-300 text-sm">{s.customer_name || '—'}</td>
                      <td className="px-4 py-3 text-gray-300 text-sm">{fmtMoney(s.total_amount, s.currency)}</td>
                      <td className="px-4 py-3 text-gray-300 text-sm">{fmtDate(s.created_at)}</td>
                      <td className="px-4 py-3">
                        <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${
                          s.status === 'completed' ? 'bg-green-900/40 text-green-400' : 'bg-amber-900/40 text-amber-400'
                        }`}>
                          {s.status === 'completed' ? 'Yakunlangan' : s.status || 'Kutilmoqda'}
                        </span>
                      </td>
                    </tr>
                  ))}
